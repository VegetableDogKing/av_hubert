...
class AVSEHubertModel(torch.nn.Module):
    def __init__(self, ckpt_path, mapping=False, from_scratch=False, no_ssl=False, no_video=False, crossfeat=False):
        ...
        user_dir = "/home/richardleelai/jchou/project/avhubert_avse/code/av_hubert/avhubert"
    ...
    def forward(self, video_feats, noisy_feats, noisy_spec):
        ...
        if not self.mapping:
            if self.crossfeat:
                return self.blstm((feature,noisy_spec))*noisy_spec, layer_results
            else:
                return self.blstm(feature)*noisy_spec, layer_results
        else:
            return self.blstm(feature), layer_results
...
if __name__ == '__main__':
    ...
    model = AVSEHubertModel('/home/jchou/project/avhubert_avse/code/checkpoint/base_lrs3_iter5.pt', no_ssl=True)
